...
from prompts.codenames_collaborative import (
    ...
    template_gen_expert_identity_spymaster,
    template_gen_expert_identity_guesser,
    template_expert_prompting_spymaster,
    template_expert_prompting_guesser
)
import json
...
class CodenamesCollaborativeTask(Task):
    ...
    def get_input_prompt(self, idx: int, method: str, **kwargs) -> str:
        ...
        if role == 'spymaster':
            ...
            elif method == "Expertllama_expert_identity":
                input_prompt = template_gen_expert_identity_spymaster.format(n = n, target_words = target_words_str, word_list = word_list_str)
            else:
            ...
            if method == "standard":
                ...
                input_prompt = template_gen_expert_identity_guesser.format(n = n, hint_word = hint_word, word_list = word_list_str)
            else:
    ...
    def get_input_prompt_Expertllama_spymaster(self, idx: int, expert_identity: str, **kwargs) -> str:
        datapoint = self.data[idx]
        word_list = datapoint['word_list']
        word_list_str = ", ".join(word_list)
        target_words = datapoint['target_words']
        ...
        n = len(target_words)
        input_prompt = template_expert_prompting_spymaster.format(expert_identity = expert_identity, n = n, target_words = target_words_str, word_list = word_list_str)
        
        return input_prompt

    def get_input_prompt_Expertllama_guesser(self, idx: int, expert_identity: str, **kwargs) -> str:
        datapoint = self.data[idx]
        word_list = datapoint['word_list']
        word_list_str = ", ".join(word_list)
        target_words = datapoint['target_words']
        ...
        n = len(target_words)
        input_prompt = template_expert_prompting_guesser.format(expert_identity = expert_identity, n = n, hint_word = hint_word, word_list = word_list_str)
    ...
    def prompt_unwrap(response: str, method: str):
        ...
        if method in ["standard", "cot"]:
            ...
            #Final answer更靠后
            if len(response.split("<|im_start|>")) >= 3:
                return response.split("<|im_start|>")[3], True
            else:

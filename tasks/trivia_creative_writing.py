...
from prompts.trivia_creative_writing import standard_prompt, cot_prompt, spp_prompt, spp_prompt_profile, spp_prompt_fixed_persona, template_gen_expert_identity, template_expert_prompting
import json
...
class TriviaCreativeWritingTask(Task):
    ...
    def get_input_prompt(self, idx: int, method: str, **kwargs) -> str:
        ...
        if method == "standard":
            ...
            input_prompt = template_gen_expert_identity.format(n=n, questions=questions_str, topic=topic)
        else:
            raise NotImplementedError(f"method {method} not implemented")
        
        return input_prompt

    def get_input_prompt_Expertllama(self, idx: int, expert_identity: str, **kwargs) -> str:
        datapoint = self.data[idx]
        questions = datapoint["questions"]
        topic = datapoint["topic"]
        n = len(questions)
        ...
        input_prompt = template_expert_prompting.format(n=n, questions=questions_str, topic=topic, expert_identity = expert_identity)
